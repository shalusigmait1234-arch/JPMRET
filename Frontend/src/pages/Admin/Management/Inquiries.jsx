      ...
      <div className="grid grid-cols-1 gap-4">
              ...
              <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6">
                <div className="space-y-4 flex-1">
                  <div className="flex items-center space-x-3">
                    <div className="h-10 w-10 rounded-full bg-[#f4f7fa] flex items-center justify-center text-[#001e38] font-bold text-sm">
                      {inquiry.name.charAt(0)}
                    </div>
                    <div>
                      <h4 className="text-base font-bold text-[#001e38]">{inquiry.name}</h4>
                  ...
                  <div className="bg-[#f9f9f9] p-4 rounded-lg">
                    <h5 className="text-xs font-black text-[#bd9143] uppercase tracking-widest mb-2">{inquiry.subject}</h5>
                ...
                <div className="flex flex-col items-end space-y-4 min-w-[150px]">
                  <div className="text-right">
                    <p className="text-[10px] text-gray-400 font-bold flex items-center justify-end">
                      <Calendar size={12} className="mr-1" /> {new Date(inquiry.createdAt).toLocaleDateString()}
                    </p>
                    <p className="text-[10px] text-gray-400 font-bold flex items-center justify-end mt-1">
                      <Clock size={12} className="mr-1" /> {new Date(inquiry.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>

                  <div className="flex flex-col space-y-2 w-full">
                    <span className={`text-center py-1.5 px-3 rounded-md text-xs font-black uppercase tracking-widest ${inquiry.status === 'New' ? 'bg-blue-100 text-blue-700' :
                    ...
                    <div className="flex space-x-2">
                      {inquiry.status === 'New' && (
                        <button
                          ...
                          className="flex-1 flex items-center justify-center p-2 bg-[#001e38] text-white rounded-md hover:bg-[#bd9143] transition-all text-xs font-bold uppercase tracking-tighter"
                        >
                          <CheckCircle size={14} className="mr-1" /> Mark Read
                        </button>
                      )}
                      <button className="flex-1 flex items-center justify-center p-2 border border-[#001e38] text-[#001e38] rounded-md hover:bg-gray-50 transition-all text-xs font-bold uppercase tracking-tighter">
        ...
        {inquiries?.length === 0 && (
          <div className="bg-white p-20 rounded-xl border border-dashed border-gray-200 text-center">
            ...
            <p className="text-base text-gray-400 font-bold uppercase tracking-widest">No inquiries found in the database</p>
          </div>
        )}
      </div>
    </div>
